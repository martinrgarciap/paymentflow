...
const FlagButton = ({
  ...
  <div className="flex items-end justify-between mt-3">
    <p
      className={`text-[10px] font-semibold uppercase tracking-wide transition-colors duration-200 ${active ? "text-white/80" : "text-gray-400"}`}
    >
      {label}
    </p>
    {flaggedCount !== undefined && flaggedCount > 0 && (
      <span
        className={
          "text-[10px] font-semibold px-1.5 py-0.5 rounded-full inline-flex items-center gap-0.5 shrink-0 ml-1 bg-red-100 text-red-500"
        }
        onClick={(e) => {
          e.stopPropagation();
          onClick?.();
...
export default function StatCard({
  ...
  return (
    ...
    >
      ...
      <div className="relative px-2 py-4 hidden md:flex md:flex-col md:justify-between h-full">
      ...
      <div className="relative px-3 py-3 md:hidden flex flex-col justify-between h-full">
